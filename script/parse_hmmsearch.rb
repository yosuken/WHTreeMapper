...
--min-hmm-len [0](int)              <minimum length of hmmlen of linked hit>
--min-hmm-cov [0](float, 0-1)       <minimum fraction of hmmlen of linked hit in hmm length>
--min-ali-len [0](int)              <minimum length of alilen of linked hit>
--min-ali-cov [0](float, 0-1)       <minimum fraction of alilen of linked hit in protein length>
--min-hmm-len-dom [0](int)          <minimum length of hmmlen of domain hit>
--min-hmm-cov-dom [0](float, 0-1)   <minimum fraction of hmmlen of domain hit in hmm length>
--min-ali-len-dom [0](int)          <minimum length of alilen of domain hit>
--min-ali-cov-dom [0](float, 0-1)   <minimum fraction of alilen of domain hit in protein length>
...
while i < ARGV.size 
  ...
  when "--min-hmm-len-dom"
    minhmmlendom = ARGV[i+1]; i += 1
  when "--min-hmm-cov-dom"
    minhmmcovdom = ARGV[i+1]; i += 1
  when "--min-ali-len-dom"
    minalilendom = ARGV[i+1]; i += 1
  when "--min-ali-cov-dom"
    minalicovdom = ARGV[i+1]; i += 1
...
[e_thre, minhmmcov, minalicov, minhmmcovdom, minalicovdom, maxaliovpfrc, maxhmmovpfrc].zip(
  %w|evalue min-hmm-cov min-ali-cov min-hmm-cov-dom min-ali-cov-dom max-ali-ovp-frc-dom max-hmm-ovp-frc-dom|){ |var, str|
  raise("'--#{str} #{var}' is not valid. Must be float") unless var.is_float?
}
[minhmmlen, minalilen, minhmmlendom, minalilendom, maxaliovplen, maxhmmovplen].zip(
  %w|min-hmm-len min-ali-len min-hmm-len-dom min-ali-len-dom max-ali-ovp-len-dom max-hmm-ovp-len-dom|){ |var, str|
...
MinHmmLenDom = minhmmlendom.to_i
MinAliLenDom = minalilendom.to_i
MinHmmCovDom = minhmmcovdom.to_f
MinAliCovDom = minalicovdom.to_f
...
while l = fr.gets
  if l =~ /^Query:\s+(\S+)\s+\[M=(\d+)\]/  # detect new entry
    ...
    next if hmmlen < MinHmmLenDom
    next if alilen < MinAliLenDom
    next if hmmlen.to_f / hmm2len[hmm] < MinHmmCovDom
    next if alilen.to_f / gid2len[gid] < MinAliCovDom
...
gids.each_key{ |gid|
  ...
  idx2ali_range = {} ### h[0] = 10..230, h[1] = 10..230, h[2] = 10..230, h[3] = 450..540, h[4] = 450..540 ### merged ali range
  idx2hmm_range = {} ### h[0] = 10..230, h[1] = 10..230, h[2] = 10..230, h[3] = 450..540, h[4] = 450..540 ### merged ali range
  link_cands.each_key{ |hmm|
    ...
    (0..n-2).each{ |i|
      ...
      if intersect
        ...
        if ovp_len > MaxHmmOvpLen or ovp_frc > MaxHmmOvpFrc
          flag = 1
        end
      end
    ...
    links.each.with_index{ |link, idx|
      ...
      hmm_fm = elems.map{ |i| cands[i][5].to_i }.min ### min of hmm.fm
      hmm_to = elems.map{ |i| cands[i][6].to_i }.max ### max of hmm.to
      ali_fm = elems.map{ |i| cands[i][7].to_i }.min ### min of ali.fm
      ali_to = elems.map{ |i| cands[i][8].to_i }.max ### max of ali.to
      ...
      elems.each{ |i|
        ...
        idx2link[j]  = idx + 1
        idx2ali_range[j] = ali_fm..ali_to
        idx2hmm_range[j] = hmm_fm..hmm_to
      }
    }
  }
  ...
  binfos.each.with_index{ |info, idx|
    ...
    if link = idx2link[idx]
      ali_fm = idx2ali_range[idx].min
      ali_to = idx2ali_range[idx].max
      hmm_fm = idx2hmm_range[idx].min
      hmm_to = idx2hmm_range[idx].max
      link_lab = "link#{link}_#{hmm}"
    end

    hmmlen = hmm_to - hmm_fm + 1
    alilen = ali_to - ali_fm + 1

    ### linked hit length check
    hlen = hmm2len[hmm]
    next if hmmlen < MinHmmLen
    next if alilen < MinAliLen
    next if hmmlen.to_f / hlen < MinHmmCov
    next if alilen.to_f / glen < MinAliCov

    ### write output
    name   = "#{gid}_fm#{ali_fm}_to#{ali_to}"
    o = pref + info[1..-1] + [link_lab, name]
    ...
    fwbF.puts ">#{labF}\n#{gid2seq[gid][fm..to]}" unless labFs[labF]
    fwbW.puts ">#{labW}\n#{gid2seq[gid]}" unless labWs[labW]

    ### store label to avoid duplication in fasta file
    labFs[labF] = 1
    labWs[labW] = 1
  }
}
