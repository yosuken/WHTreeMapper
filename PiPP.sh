...
[1;32m[options][0m
  ...
  (prefilter)
    ...
    --minhmmlenfrc   [float between 0 and 1] (default: 0) -- minimum fraction of hmm length in alignment of hmmsearch
    --trim-option    [merge or largest] (default: merge)  -- take merged regions of hmmsearch hits (if multiple) or take largest hit

  (align)
    --mafft-method   [FFT-NS-2|FFT-NS-i|E-INS-i] (default: E-INS-i) -- mafft method used with '--add' and '--keeplength'.
...
minhmmlenfrc="${minhmmlenfrc:-0}"
trim_option="${trim_option:-merge}"
mafft_method="${mafft_method:-E-INS-i}"
...
elif [[   "$mafft_method" != "FFT-NS-2" ]] && [[ "$mafft_method" != "FFT-NS-i" ]] && [[ "$mafft_method" != "E-INS-i" ]] ; then
	echo "$Errmsg  --mafft-method should be either FFT-NS-2 or FFT-NS-i or E-INS-i ('$mafft_method' is given)." 1>&2; echo "         execute '$PROGNAME -h' for usage" 1>&2; exit 1
elif [[ ! $ncpus =~ $re ]] ; then
...
rake -f $rakefile \
	...
	evalue="$evalue" minseqlen="$minseqlen" minhmmlen="$minhmmlen" minhmmlenfrc="$minhmmlenfrc" \
